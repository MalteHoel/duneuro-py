...
dipoles = np.empty((1, 6), dtype = np.float64)
dipoles[0, :3] = dipole_position
dipoles[0, 3:] = dipole_moment
print('Data loaded')
...
eeg_transfer_matrix_raw, computation_information = meeg_driver.computeEEGTransferMatrix(driver_cfg)
eeg_transfer_matrix = np.array(eeg_transfer_matrix_raw)
print('Transfer matrix computed')

# if the transfer matrix has already been computed, you can just load the transfer matrix
# instead of recomputing it

dipoles_duneuro = [dp.Dipole3d(dipole[0:3], dipole[3:6]) for dipole in dipoles]
...
numerical_potentials, computation_information = meeg_driver.applyEEGTransfer(eeg_transfer_matrix, dipoles_duneuro, driver_cfg)
numerical_solution = numerical_potentials[0]
numerical_solution -= np.mean(numerical_solution)
...
if rel_error > tolerance:
  raise Exception(f'Relative error not within tolernace')
